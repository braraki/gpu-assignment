...
OUTPUT_ROOT="${OUTPUT_ROOT:-${WORKSPACE_ROOT}/results/part4-qkv-norm-rope-vnorm-kvcache-fusion/microbench/compiler_dumps}"
RUN_DIR="${OUTPUT_ROOT}/${RUN_NAME}"
BENCHMARK_OUTPUT_DIR="${RUN_DIR}/benchmark_outputs"
LOG_PATH="${RUN_DIR}/torch_output_code.log"

case "$PROVIDER" in
  baseline_compiled|post_gemm_kvcache_compiled)
    ;;
  *)
    echo "PROVIDER must be baseline_compiled or post_gemm_kvcache_compiled, got: ${PROVIDER}" >&2
    exit 1
    ;;
esac

activate_vllm_venv
if [[ "$PROVIDER" == "post_gemm_kvcache_compiled" ]]; then
